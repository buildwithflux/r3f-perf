  ...
  raf(() => {
    ...
    const arrData = [
      cg.lineStrip(fps.pointsX, fps.pointsY, {
        colors: colorBlind ? [100 / 255, 143 / 255, 255 / 255, 1] : [238 / 255, 38 / 255, 110 / 255, 1],
        widths: 1.5,
      }),
    ]
    ...
    if (trackCPU) {
      ...
      cg.lineStrip(cpu.pointsX, cpu.pointsY, {
        colors: colorBlind ? [254 / 255, 254 / 255, 98 / 255, 1] : [66 / 255, 226 / 255, 46 / 255, 1],
        widths: 1.5,
      })
    }
    ...
    if (trackGPU) {
      ...
      arrData.push(
        cg.lineStrip(gpu.pointsX, gpu.pointsY, {
          colors: colorBlind ? [254 / 255, 254 / 255, 255 / 255, 1] : [253 / 255, 151 / 255, 31 / 255, 1],
          widths: 1.5,
        })
      )
    }
...
export const ChartUI: FC<PerfUIProps> = ({ perfContainerRef, colorBlind, trackCPU, trackGPU }) => {
  ...
  return (
    <Graph
      ...
      />
      {!paused && cg && (
        <ChartCurve colorBlind={colorBlind} trackCPU={trackCPU} trackGPU={trackGPU} cg={cg} canvas={canvas} />
      )}
      {paused && (
        <Graphpc>
